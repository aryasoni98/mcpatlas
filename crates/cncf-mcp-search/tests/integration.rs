...
// In a real integration test, we'd import from cncf-mcp-core, but since
...
fn test_project_summary_conversion() {
    ...
    let summary = cncf_mcp_data::models::ProjectSummary::from(prometheus);
...
"#;
    ...
    let landscape = cncf_mcp_data::landscape::parse_landscape_yaml(yaml).unwrap();
...
fn test_github_url_parsing() {
    use cncf_mcp_data::github::parse_github_url;
