...
#[derive(Parser)]
#[command(name = "cncf-mcp-cli", about = "CLI companion for CNCF MCP Server")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Version => {
            println!("cncf-mcp-cli v{}", env!("CARGO_PKG_VERSION"));
        }
        Commands::Sync {
            output,
            skip_github,
            ...
            let yaml = cncf_mcp_data::landscape::fetch_landscape_yaml().await?;
            ...
            let landscape = cncf_mcp_data::landscape::parse_landscape_yaml(&yaml)?;
            let projects = cncf_mcp_data::landscape::flatten_projects(&landscape);
            println!("Synced {} projects from CNCF landscape", projects.len());

            if !skip_github {
                info!("Enriching with GitHub metrics...");
                let config = cncf_mcp_data::pipeline::PipelineConfig {
                    github_token: cli.github_token,
                    landscape_file: Some(output_path.into()),
                    github_concurrency: 5,
                    ..Default::default()
                };
                let enriched = cncf_mcp_data::pipeline::run_pipeline(&config).await?;
        ...
        Commands::Search {
            ...
            let index = cncf_mcp_search::SearchIndex::build(&projects)?;
        ...
        Commands::Graph { name, data } => {
            let projects = load_projects(&data)?;
            let graph = cncf_mcp_graph::engine::ProjectGraph::build(&projects);
        ...
        Commands::Validate { path } => {
            ...
            let (landscape, projects) =
                cncf_mcp_data::landscape::load_landscape_from_file(file_path)?;
        ...
        Commands::Serve { transport, port } => {
            ...
            println!("  cncf-mcp --transport {transport} --port {port}");
        }
    }
...
fn load_projects(data_path: &str) -> Result<Vec<cncf_mcp_data::models::Project>> {
    let path = std::path::Path::new(data_path);
    if !path.exists() {
        anyhow::bail!(
            "Data file not found: {data_path}\nRun `cncf-mcp-cli sync` first to fetch the landscape data."
        );
    }
    let (_landscape, projects) = cncf_mcp_data::landscape::load_landscape_from_file(path)?;
    Ok(projects)
}
