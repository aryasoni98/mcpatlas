...
fn sample_state() -> Arc<cncf_mcp_core::server::AppState> {
    ...
    let graph = std::sync::Arc::new(cncf_mcp_graph::engine::ProjectGraph::build(&projects));
    Arc::new(cncf_mcp_core::server::AppState {
        projects,
        search_index,
        graph,
...
async fn test_initialize() {
    ...
    let response = cncf_mcp_core::tools::handle_jsonrpc(&state, &request).await;
    assert_eq!(response["jsonrpc"], "2.0");
    assert_eq!(response["id"], 1);
    assert!(response["result"]["serverInfo"]["name"].as_str().unwrap() == "cncf-mcp");
...
async fn test_plugin_tool_registration_and_dispatch() {
    use cncf_mcp_core::tools::{register_plugin_tool, DynamicTool};
    ...
    let handler = Box::new(move |args: serde_json::Value| {
        ...
        Box::pin(async move { Ok(out) }) as Pin<Box<dyn Future<Output = anyhow::Result<serde_json::Value>> + Send>>
    });
    ...
    let list_resp = cncf_mcp_core::tools::handle_jsonrpc(&state, &list_req).await;
    let tools = list_resp["result"]["tools"].as_array().unwrap();
    let names: Vec<&str> = tools.iter().map(|t| t["name"].as_str().unwrap()).collect();
    assert!(names.contains(&"plugin_echo"), "plugin_echo should be in tools list");
    ...
    let call_resp = cncf_mcp_core::tools::handle_jsonrpc(&state, &call_req).await;
    assert!(call_resp["error"].is_null(), "plugin_echo call should succeed: {:?}", call_resp["error"]);
...
async fn test_batch_request() {
    ...
    let response = cncf_mcp_core::tools::handle_jsonrpc_message(&state, &batch).await;
    let responses = response.unwrap();
    let arr = responses.as_array().unwrap();
    assert_eq!(arr.len(), 2);
    assert_eq!(arr[0]["id"], 1);
    assert_eq!(arr[0]["result"]["serverInfo"]["name"], "cncf-mcp");
...
async fn test_resource_unsubscribe() {
    ...
    cncf_mcp_core::tools::handle_jsonrpc(&state, &request).await;
...
async fn test_cancellation_notification() {
    ...
    cncf_mcp_core::tools::handle_jsonrpc(&state, &notification).await;
...
async fn test_cancellation_unknown_request() {
    ...
    let response = cncf_mcp_core::tools::handle_jsonrpc(&state, &notification).await;
