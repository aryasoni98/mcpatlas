...
use cncf_mcp_core::server::{AppState, build_router};
...
async fn test_info_endpoint() {
    ...
    assert_eq!(info["server"], "cncf-mcp");
...
async fn test_mcp_post_initialize() {
    ...
    assert!(result["result"]["serverInfo"]["name"].as_str().unwrap() == "cncf-mcp");
}
...
async fn test_streamable_initialize_creates_session() {
    ...
    let body = response.into_body().collect().await.unwrap().to_bytes();
    let result: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(result["result"]["serverInfo"]["name"], "cncf-mcp");
}
...
async fn test_metrics_endpoint() {
    ...
    assert!(text.contains("cncf_mcp_projects_total 2"));
    assert!(text.contains("cncf_mcp_requests_total 0"));
    assert!(text.contains("cncf_mcp_uptime_seconds"));
    assert!(text.contains("cncf_mcp_graph_nodes"));
}
