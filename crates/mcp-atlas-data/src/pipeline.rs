...
use crate::summary::{SummaryProvider, project_content_hash};
    ...
    if config.artifact_hub_enabled {
        ...
        for (i, project) in projects.iter().enumerate() {
            ...
            handles.push(tokio::spawn(async move {
                ...
                match client.search_packages(&name, 0, max_packages).await {
        ...
        if let Some(ref api_base) = config.summary_api_base {
            ...
            let to_summarize: Vec<usize> = projects
                ...
                .filter(|(_, p)| {
                    ...
                    let skip = p.summary_content_hash.as_deref().is_some_and(|h| h == hash);
                    !skip
                })
                .map(|(i, _)| i)
